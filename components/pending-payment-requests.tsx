    ...
    <Card className="p-4 md:p-6 border-orange-200 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-900">
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-orange-700 dark:text-orange-200">{requests.length} yêu cầu cần xác nhận</p>
        </div>

        <div className="space-y-3">
          {requests.map((request) => (
            <Link key={request.id} href={`/debts/${request.personId}`}>
                ...
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">
                      //{request.debtor?.display_name || "User"} {" "}
